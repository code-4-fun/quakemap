...
angular.module('mapModule')
  .controller('mapController', ['$scope', 'mapService', 'uiGmapGoogleMapApi',
    'geolocation', '$http',
    function($scope, mapService, uiGmapGoogleMapApi, geolocation, $http) {
      ...
      uiGmapGoogleMapApi.then(function(maps) {
        ...
        geolocation.getLocation().then(function(data) {
          ...
          $scope.currentLocation.center.longitude = parseFloat(coords
            .long).toFixed(3);
          $scope.currentLocation.center.latitude = parseFloat(coords.lat)
            .toFixed(3);
          $scope.isMapReady = true;
          $scope.markers.push(
            mapService.createMarker(
              0, $scope.currentLocation.center.latitude, $scope.currentLocation
              .center.longitude,
  ...
  .factory('mapService', [
    function() {
      ...
      var Point = function(id, latitude, longitude,
        ...
        scope.content = '';
        scope.content = '<ul><li>Location:<ul><li>latitude: ' + scope.latitude +
          '</li><li>longitude: ' + scope.longitude + '</li></ul></ul>';
        ...
        if (severity === mapService.AlertTypes.INFO) {
          ...
          scope.content = scope.content + '<li>Details: ' + ((scope.message) ?
              scope.message : '') + '</li>' + '<li>Severity: ' + scope.severity +
            '</li>' + '<li>Magnitude: ' + scope.magnitude + '</li>' +
